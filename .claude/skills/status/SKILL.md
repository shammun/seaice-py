...
# /status
1. Read `progress.json`; print a table `chapter | analyze | port | verify | review | notebook | knowledge | publish | notes`.
   A chapter is only finished when `publish` is `pass` (Colab variant + HTML page on GitHub Pages).
