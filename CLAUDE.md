...
| `notebooks/chNN_<slug>.ipynb` (+ `build_chNN.py`) | Teaching notebook for the chapter (cells 1–3: Drive mount, clone/pull, `load_image`) |
| `notebooks/chNN_<slug>_colab.ipynb`, `notebooks/chNN_<slug>.html` | **PUBLISH phase** outputs of `tools/publish_notebook.py chNN`: Colab variant (outputs stripped) and the styled HTML page (built from a run WITHOUT `data/book`, i.e. on the public-domain substitute). Served by GitHub Pages at `https://shammun.github.io/seaice-py/` |
| `index.html`, `README.md` (index table), `assets/clean-educational.css` | Site scaffolding, regenerated by the publish tool |
| `seaice/core/public_images.py`, `data/online/SOURCES.md` | Registry of public-domain substitutes for the book's images (one per image name per chapter) + provenance; the only committed things under `data/online/` |
...
- `/do-chapter N` — the full pipeline for one chapter (analyze → port → verify → review → notebook → knowledge →
  **publish**: substitute-image registry, two headless runs, `tools/publish_notebook.py` from the public run, push, Pages check).
  Public repo: `https://github.com/shammun/seaice-py` (branch `main`), site `https://shammun.github.io/seaice-py/`.
